...
test=simHMM(data=data.frame(ch=c("+-0000","0+0000"),freq=c(100,100),stringsAsFactors=FALSE),model="HMMCJS1tl",initial=list(Phi=1,p=1,tau=.1))
notperm=sample.int(nrow(test),.1*nrow(test),replace=FALSE)
test$perm="P"
test$perm[notperm]="N"
test$perm=factor(test$perm)
test$ch[test$perm=="N"]=gsub("-","0",test$ch[test$perm=="N"])
dp=process.data(test,model="HMMCJS1tl",groups="perm")
ddl=make.design.data(dp)
ddl$p$fix[ddl$p$group=="N"&ddl$p$stratum=="0"]=0
xx=crm(dp,ddl,hessian=T,accumulate=F)
xx

test=simHMM(data=data.frame(ch=c("+-0000","0+0000"),freq=c(100,100),stringsAsFactors=FALSE),model="HMMCJS1tl")
dp=process.data(test,model="HMMCJS1tl")
ddl=make.design.data(dp)
xx=crm(dp,ddl,hessian=T,accumulate=F)
xx
